...
% Extend Kalman Filter initialization
P = diag([1e4, 1e4, 1e4, 100, 100, 100, 1e4, 100]);
...
%% Find pseudoranges ======================================================
    ...
    navSolutions.rawD(:, currMeasNr) = doppler(trackResults, currMeasSample, activeChnList, settings);
...
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        [xyzdt, velxyzddt,navSolutions.el(activeChnList, currMeasNr), ...
               navSolutions.az(activeChnList, currMeasNr), ...
               navSolutions.DOP(:, currMeasNr)] =...
                       weightedLeastSquare(satPositions, satVelocity, clkCorrRawP, clkCorrRawD, settings);
        % Calculate receiver velocity
        % velxyzddt = weightedLeastSquareVelocity(satPositions, satVelocity, clkCorrRawD, xyzdt, settings);
        % velxyzddt = testt(satPositions, clkCorrRawP, settings,satVelocity,clkCorrRawD);
        ...
        navSolutions.velX(currMeasNr) = velxyzddt(1);
        navSolutions.velY(currMeasNr) = velxyzddt(2);
        navSolutions.velZ(currMeasNr) = velxyzddt(3);
        ...
        [X1, P1] = extendKalmanFilter(satPositions, satVelocity, clkCorrRawP, clkCorrRawD, X, P, Q, settings);
        navSolutions.X_ekf(currMeasNr) = X1(1);
        navSolutions.Y_ekf(currMeasNr) = X1(2);
        navSolutions.Z_ekf(currMeasNr) = X1(3);
        navSolutions.velX_ekf(currMeasNr) = X1(4);
        navSolutions.velY_ekf(currMeasNr) = X1(5);
        navSolutions.velZ_ekf(currMeasNr) = X1(6);
        X = X1;
        P = P1;
...
%% Coordinate conversion ==================================================
        ...
        navSolutions.velX(currMeasNr)        = NaN;
        navSolutions.velY(currMeasNr)        = NaN;
        navSolutions.velZ(currMeasNr)        = NaN;
