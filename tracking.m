...
trackResults.I_I            = zeros(11, settings.msToProcess);
...
trackResults.Q_Q            = zeros(11, settings.msToProcess);
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:codePeriods
            ...
            thecode = [];
            % Define index into code vectors
            for i = 1:11
                tcode = (remCodePhase - 0.1*(6-i): codePhaseStep: ...
                    ((blksize-1)*codePhaseStep+remCodePhase)-0.1*(6-i));
                tcode2 = ceil(tcode) + 1;
                thecode(i, :) = caCode(tcode2);
            end
            ...
            % Get the code correlator
            I_I = [];
            Q_Q = [];
            for i = 1:11
                I_I(i, :) = sum(thecode(i,:) .* iBasebandSignal);
                Q_Q(i, :) = sum(thecode(i,:) .* qBasebandSignal);
            end
            ...
            trackResults(channelNr).I_I(:,loopCnt) = I_I;
            trackResults(channelNr).Q_E(loopCnt) = Q_E;
            trackResults(channelNr).Q_P(loopCnt) = Q_P;
            trackResults(channelNr).Q_L(loopCnt) = Q_L;
            trackResults(channelNr).Q_Q(:,loopCnt) = Q_Q;
